...
void CLevelSystem::CreateDefaultLevel()
{
	CreateLevel("Menu");
}
	...
	CGame::Get().GetLogicalSystem()->GetActorSystem()->ForEachPlayer([this](CPlayer* pPlayer)
		{
			...
			{
				if (pController->GetType() != CController::Network || static_cast<CNetworkController*>(pController.get())->GetClientId() != -1)
				{
					SPlayerInfo info;
...
SmartId CLevelSystem::SpawnPlayer(const std::string& player, const std::shared_ptr<CController>& pController)
{
	if (!m_pLevelConfig)
	{
		return InvalidLink;
